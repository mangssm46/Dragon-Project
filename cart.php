	...
	if(!empty($_GET["pricediscount"])){
 	 ...
 	 if(isset($_GET["Action"])){ 
		 ...
		 {  
			...
			$total = 0;
			$sql2 = "SELECT * FROM list_order where pk = '".$_GET["CusID"]."'  ";
			...
			$total_check = 0;
			$sql2 = "SELECT * FROM product_list where pk = '".$_GET["Menuid"]."'  ";
			$query2 = mysqli_query($objCon,$sql2);
			while($objResult2 = mysqli_fetch_array($query2))
			{
			...
			$cal_total = $total_check + $total;
			$strSQL = " Update product_list Set total = '".$cal_total."'  " ;
			$strSQL .=" WHERE pk = '". $_GET["Menuid"] ."' "; 
			...
			$objQuery = mysqli_query($objCon,$strSQL); 
				...
				echo("<script>window.location = 'cart.php?pricediscount=".$pricediscount."&codeno=".$codeno."';</script>"); 
		 } 
	 }
...
?>  
    ...
    <section class="container py-5">
        <div class="row text-center pt-3">
         ...
         <div class="row">
		 <div class="col-lg-12" >
		 <form action="save_confrim.php" method="get" enctype="multipart/form-data">
		 ...
		 <input type="hidden" id="showtxtdata" name="showtxtdata" value="11111" >
                ...
                <div class="col-lg-12" align="center">
                 ...
                 <button type="button" class="btn btn-sm  " style="background-color: #FF6633; border: 1px solid #FF6633; margin-bottom: 30px; margin-top: 40px; width: 200px;   margin-right: 10px; "   > <font color="#FFF" style="font-size: 18px; ">  ตะกร้าสินค้าปกติ  </font> </button>
				 </a> 
                	
                 <a href="cart_pre.php"  style="text-decoration-line: none;"     > 
                 <button type="button" class="btn btn-sm  " style="background-color: #FFF; border: 1px solid #FF6633; margin-bottom: 30px; margin-top: 40px;   margin-right: 10px;  width: 200px; "   > <font color="#FF6633" style="font-size: 18px; ">  ตะกร้าสินค้าพรีออเดอร์  </font> </button>
				 </a> 
                </div>
            	...
            	<?php   
					...
					$countall = 0;
					$sql2 = "SELECT * FROM list_order where create_by = '".$_SESSION["UserID2"] ."' and bill_no = '' and status_product = 'Product' ";
					$query2 = mysqli_query($objCon,$sql2); 
					while($objResult2 = mysqli_fetch_array($query2))
					{ 
						$countall++;; 
					...
					$sql = "SELECT * FROM list_order where create_by = '".$_SESSION["UserID2"] ."' and bill_no = '' and status_product = 'Product' order by pk desc "; 
					...
					{ 
						...
						if($objResult["status_product"] == "Product"){
							$img = "";
							$sql2 = "SELECT a.*, b.name, b.typedata2 FROM product_list a 
				...
				<div class="grid-item">       
				 <font color="black" size="3px" class="serif">  
				  <div style="margin-left: 10px;">
				   ...
				   <?php if(empty($img)){ ?>
					<img src="img/p1.png" alt="product" class="img-fluid " style="height: 100px; ">
				   <?php }else{ ?>
						...
						function IsNumeric<?php echo $i; ?>()
						{
							...
							$.ajax({
								...
								if(check_number == 0){
									...
									window.location.href = "cart.php?pricediscount="+pricediscount+"&codeno="+codeno;
								}
			...
			<?php echo number_format(0+$totalall1+100); ?>
			</font> 
			</div> 
