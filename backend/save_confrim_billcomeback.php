...
include('../database.php');
	...
	if(empty($create_by)){
		...
		$bill_no = ""; 
		$GGyear= date('Y')+543 ; 
		$sql2 = "SELECT count(pk) as total FROM run_billcomeback  ";
		...
		$bill_no =  "DDT".date('Ymd')."-".$total; 
		$strSQL = "INSERT INTO run_billcomeback (bill_no) VALUES  ('".$bill_no."')";  
		...
		if(isset($_FILES["picupload"]))
		{
		 ...
		 echo("<script>window.location = 'checkcomeback.php';</script>");
